...
import { useGetPaidPurchases, useCancelInvestment } from '../queries';
import { useQueryClient } from '@tanstack/react-query';
import toast from 'react-hot-toast';

const PurchaseTracking: React.FC = () => {
  const { data: paidPurchases, isLoading, error } = useGetPaidPurchases();
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-red-600">Error al cargar compras con fondo</p>
      </div>
    );
  }
  ...
  if (!paidPurchases || paidPurchases.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-xl font-bold text-gray-800 mb-2">No hay compras con fondo</h3>
        <p className="text-gray-600">Las compras pagadas con fondo acumulado aparecerán aquí</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">💰 Compras con Fondo Acumulado</h2>
        <p className="text-gray-600">Productos comprados directamente con fondo del cliente</p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {paidPurchases.map((investment: any) => {
              ...
              <div className="p-6">
                ...
                <div className="space-y-3 mb-4">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-bold text-green-600">{investment.monto_total.toLocaleString('es-PY')} Gs</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="text-xs text-gray-500">
                    Comprado: {new Date(investment.createdAt).toLocaleDateString('es-PY')}
                  </div>
                  <button
